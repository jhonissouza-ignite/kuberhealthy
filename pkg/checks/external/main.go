...
func (ext *Checker) cleanup() {
	...
	podList, err := podClient.List(nil, metav1.ListOptions{
...
func (ext *Checker) evictPod(podName string, podNamespace string) error {
	...
	err := podClient.Evict(nil, eviction)
	if err != nil {
...
func (ext *Checker) startPodWatcher(listOptions metav1.ListOptions, ctx context.Context) watch.Interface {
	...
	for {
		...
		watcher, err := podClient.Watch(nil, listOptions)
...
func (ext *Checker) deletePod(podName string) error {
	...
	err := podClient.Delete(nil, podName, metav1.DeleteOptions{
...
func (ext *Checker) waitForAllPodsToClear() chan error {
	...
	go func() {
		...
		for {
			...
			p, err := podClient.Get(nil, ext.podName(), metav1.GetOptions{})
...
func (ext *Checker) waitForPodExit() chan error {
	...
	go func() {
		...
		for {
			...
			// start a new watch request
			pods, err := podClient.List(nil, metav1.ListOptions{
				LabelSelector: kuberhealthyRunIDLabel + "=" + ext.currentCheckUUID,
			})
...
func (ext *Checker) waitForPodStart() chan error {
	...
	go func() {
		...
		for {
			...
			// start watching
			watcher, err := podClient.Watch(nil, metav1.ListOptions{
				LabelSelector: kuberhealthyRunIDLabel + "=" + ext.currentCheckUUID,
			})
			if err != nil {
				outChan <- err
...
func (ext *Checker) createPod() (*apiv1.Pod, error) {
	...
	return ext.KubeClient.CoreV1().Pods(ext.Namespace).Create(nil, p, metav1.CreateOptions{})
}
